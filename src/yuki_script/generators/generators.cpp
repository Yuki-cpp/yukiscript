...
{
	auto tokens = yuki::script::utils::separate_tokens(root_token);

	if(tokens.size() == 3 && yuki::script::utils::is_binary_operator(tokens[0]))
	{
		return generate_binary_op_stmt(tokens);
	}
	else if(tokens.size() == 2 && yuki::script::utils::is_unary_operator(tokens[0]))
	{
		return generate_unary_op_stmt(tokens);
	}
	else if(tokens.size() == 1 && yuki::script::utils::is_literal(tokens[0]))
	{
		return generate_literal_stmt(tokens);
	}
	else if(tokens.size() == 1 && yuki::script::utils::is_user_defined(tokens[0]))
	{
		return generate_get_var(tokens);
	}
	else if(tokens.size() == 2 && yuki::script::utils::is_user_defined(tokens[0]))
	{
...
std::unique_ptr<yuki::statements::executable_stmt> yuki::generators::generate_executable_stmt(const std::string& root_token)
{
	...
	else if(tokens.size() == 3 && tokens[0] == "set" && yuki::script::utils::is_user_defined(tokens[1]))
	{
	...
	else if(tokens.size() == 4 && tokens[0] == "function" && yuki::script::utils::is_user_defined(tokens[1]))
	{
