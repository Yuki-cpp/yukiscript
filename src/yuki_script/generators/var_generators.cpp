...
{
	assert(tokens.size() == 1);
	assert(yuki::script::utils::is_literal(tokens[0]));

	yuki::type::variable var;

	if(yuki::script::utils::is_int_literal(tokens[0]))
	{
		var = yuki::type::variable(std::atoi(tokens[0].c_str()));
	}
	else if(yuki::script::utils::is_double_literal(tokens[0]))
	{
		var = yuki::type::variable(std::atof(tokens[0].c_str()));
	}
	else if(yuki::script::utils::is_bool_literal(tokens[0]))
	{
		var = tokens[0] == "true" ? yuki::type::variable(true) : yuki::type::variable(false);
	}
	else if(yuki::script::utils::is_char_literal(tokens[0]))
	{
		var = yuki::type::variable(yuki::script::utils::as_char(tokens[0]));
	}
	else if(yuki::script::utils::is_string_literal(tokens[0]))
	{
		std::string tmp = tokens[0];
		yuki::script::utils::replace_special_char(tmp);
